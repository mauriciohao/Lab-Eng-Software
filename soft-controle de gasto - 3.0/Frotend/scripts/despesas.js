  ...
  fetch(`${API_BASE}/groups/${grupoId}/expenses`)
    ...
    .then(dados => {
      ...
      dados.data.forEach(d => {
        let valorDivisao = '-';
        if (d.amount && d.total_members && d.total_members > 0) {
          valorDivisao = 'R$ ' + (d.amount / d.total_members).toFixed(2);
        }
        ...
        tr.innerHTML = `
          ...
          <td>${d.status}</td>
          <td>
            <select class="form-select form-select-sm status-select" data-id="${d.id}">
              <option value="pendente" ${d.status === 'pendente' ? 'selected' : ''}>Pendente</option>
              <option value="pago" ${d.status === 'pago' ? 'selected' : ''}>Pagou</option>
            </select>
          </td>
        `;
      ...
      document.querySelectorAll('.status-select').forEach(select => {
        select.addEventListener('change', async function() {
          ...
          try {
            const resp = await fetch(`${API_BASE}/expenses/${id}/status`, {
              method: 'PUT',
              headers: { 'Content-Type': 'application/json' },
